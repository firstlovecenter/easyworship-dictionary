...
if(!is_numeric($cID)){
	$sql2 = "SELECT * FROM `dict` WHERE `word`='".$_GET["wid"]."' AND word LIKE '%".$_GET["wid"]."%'"; 
	...
	$sql2 = "SELECT * FROM `dict` WHERE `word`=(SELECT `word` FROM `dict` WHERE id='".$_GET["wid"]."')"; 

	$sql_result2 = mysqli_query($conn, $sql2) or die ('request "Could not execute SQL query" '.$sql2);

	$sql = "SELECT `word` FROM `dict` WHERE id='".$_GET["wid"]."'"; 
	...
	<u id="word" style="color:#FF9900; text-align: center;"><?php echo ucwords(strtolower($word));?>:</u><br>
		...
		<?php
			while($row2 = mysqli_fetch_assoc($sql_result2)){
				echo '<li>'.str_replace( array('{{','}}') , array('<span style="color: #F0FF00;">','</span>')  , str_replace(array('[[',']]'),'',$row2['meaning'])).'</li>';
			}
	   ?>
	</ul>
</h1></div>
